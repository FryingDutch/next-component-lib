...
const Curtain = ({
  ...
  return (
    <div id={id} className={cn("flex flex-col", className)}>
      <div className="flex flex-row border-y border-primary-contrast text-buttont-primary bg-primary justify-center items-center">
        <HeroIconActionButton
          icon={Bars3Icon}
          onClick={() => setOpen(!open)}
          className={cn("w-full", buttonClassName)}
          />
          {label && (<H3 content={label} className="absolute content-center text-white"/>)}
      </div>
